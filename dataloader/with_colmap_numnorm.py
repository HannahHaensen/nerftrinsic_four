...
def load_imgs(image_dir, img_ids, res_ratio):
    '''
...
def load_split(scene_dir, img_dir, data_type, num_img_to_load, skip, c2ws,
    ...
    if num_img_to_load == -1:
        ...
        for i in range(len(H)):
            print(H[i], W[i])
            if str(H[i])+str(W[i]) not in HWFocal:
                HWFocal[str(H[i])+str(W[i])] = [focal[i], len(HWFocal)]
    ...
    # load images
    if load_img:
        imgs, img_names = load_imgs(img_dir, img_ids, res_ratio)  # (N, H, W, 3) torch.float32
    else:
...
class DataloaderParameterLearning:
    ...
    def __init__(self, base_dir, scene_name, data_type, res_ratio, num_img_to_load, skip, use_ndc, load_img=True):
        ...
        # print(self.num_img_to_load, self.skip)
        ...
        split_results = load_split(self.scene_dir, self.img_dir, self.data_type, self.num_img_to_load,
                                   self.skip, self.c2ws, self.H, self.W, self.load_img, self.focal, self.res_ratio)
        ...
        self.focal = split_results['focal']  #
        self.H = split_results['H']  #
        self.W = split_results['W']  #
